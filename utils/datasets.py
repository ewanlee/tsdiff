...
def prepare_pdb2(scn_dir, data_path):
    ...
    print(
        "total n_raw_data: %d, cnt_seg: %d, cnt_success: %d"
        % (n_raw_data, cnt_seg, cnt_success)
    )
    ...
    with open(
        os.path.join(data_path, "train_data_%dk.pkl" % (n_data // 1000)), "wb"
    ) as fout:
...
def prepare_pdblarge(scn_dir, data_path):
    ...
    print(
        "total n_raw_data: %d, cnt_seg: %d, cnt_success: %d, max_residue: %d"
...
def prepare_pdb_valtest(scn_dir, data_path):
    ...
    print(
        "total n_raw_data: %d, cnt_success: %d, max_residue: %d"
        % (n_raw_data, cnt_success, max_residue)
    )
    ...
    print(
        "number of val samples: %d | number of test samples: %d | number of fails: %d"
        % (n_val, n_test, cnt_fail)
    )

    os.makedirs(os.path.join(data_path), exist_ok=True)

    with open(
        os.path.join(data_path, "val_data_%dk.pkl" % (n_val // 1000)), "wb"
    ) as fout:
        pickle.dump(all_data[:n_val], fout)
    print("save val %dk done" % (n_val // 1000))

    with open(
        os.path.join(data_path, "test_data_%dk.pkl" % (n_test // 1000)), "wb"
    ) as fout:
        pickle.dump(all_data[n_val:], fout)
...
def pdb_to_data(pdb_path, name):
    ...
    if (
        is_sidechain.sum().item() == 0
    ):  # protein built solely on GLY can not be used for sidechain prediction
        return None
...
def generate_ts_data(r_smarts, p_smarts, xyz_block, data_cls=Data):
    r = Chem.MolFromSmarts(r_smarts)
    p = Chem.MolFromSmarts(p_smarts)
    ...
    for atom in np.array(r.GetAtoms())[r_perm_inv]:
        r_atomic_number.append(atom.GetAtomicNum())
        r_aromatic.append(1 if atom.GetIsAromatic() else 0)
        hybridization = atom.GetHybridization()
        r_sp.append(1 if hybridization == HybridizationType.SP else 0)
        r_sp2.append(1 if hybridization == HybridizationType.SP2 else 0)
        r_sp3.append(1 if hybridization == HybridizationType.SP3 else 0)

    for atom in np.array(p.GetAtoms())[p_perm_inv]:
        p_atomic_number.append(atom.GetAtomicNum())
        p_aromatic.append(1 if atom.GetIsAromatic() else 0)
        hybridization = atom.GetHybridization()
        p_sp.append(1 if hybridization == HybridizationType.SP else 0)
        p_sp2.append(1 if hybridization == HybridizationType.SP2 else 0)
        p_sp3.append(1 if hybridization == HybridizationType.SP3 else 0)
...
def rdmol_cluster_to_data(mol: Mol, smiles=None):
    ...
    for i, subgraph in enumerate(
        nx.connected_components(to_networkx(data, to_undirected=True))
    ):
...
def preprocess_iso17_dataset(base_path):
    ...
    print(
        "Train | find %d molecules with %d confs"
        % (len(smiles_set_train), len(all_train))
    )
    ...
    print(
        "Test | find %d molecules with %d confs" % (len(smiles_set_test), len(all_test))
    )
...
def preprocess_GEOM_dataset(
    ...
    val_size = test_size = (1.0 - train_size) / 2
    ...
    for i in tqdm(range(len(pickle_path_list))):
        ...
        if mol.get("uniqueconfs") <= max_conf:
            ...
            all_weights = np.array(
                [_.get("boltzmannweight", -1.0) for _ in mol.get("conformers")]
            )
...
def preprocess_GEOM_dataset_with_fixed_num_conf(
    base_path,
    dataset_name,
    conf_per_mol=5,
    train_size=0.8,
    ...
    assert len(pickle_path_list) >= tot_mol_size, (
    ...
    print(
        "pre-filter: find %d molecules with %d confs, use %d molecules with %d confs"
    ...
    for i in range(
        ...
        int(len(split_indexes) * (train_size + val_size)),
    ):
        index2split[split_indexes[i]] = "val"
    for i in range(
        int(len(split_indexes) * (train_size + val_size)), len(split_indexes)
    ):
...
def get_test_set_with_large_num_conf(
    base_path,
    dataset_name,
    block,
    tot_mol_size=1000,
    ...
    print(
        "poster-filter: find %d molecules with %d confs"
        % (num_valid_mol, num_valid_conf)
    )
